...
function Diskusjoner() {
  const [articles, setArticles] = useState([]);

  useEffect(() => {
    const fetchArticles = async () => {
      const q = query(collection(db, 'articles'), where('category', '==', 'diskusjoner'), orderBy('createdAt', 'desc'));
  ...
  return (
    <div className="container mx-auto p-4 pt-20">
      ...
      <main className="pt-20 pb-24 mt-16 min-h-screen">
        <h2 className="text-3xl font-bold mb-6 text-center">Diskusjoner</h2>
        <h4 className="text-xl font-semibold mb-4">Siste diskusjoner</h4>
